...
//        };//O/P-19
        ...
        System.out.println(pq.findLowestPriceWay2nd(products,discounts));
    }
    ...
    private int findLowestPriceWay2nd(List<List<String>> products, List<List<String>> discounts) {
        HashMap<String,Pair<String ,String>> map = new HashMap<>();
        for(List<String > x:discounts){
            map.put(x.get(0),new Pair<>(x.get(1),x.get(2)));
        }
        ...
        for(List<String> product:products){
            ...
            for(int i=1;i<product.size();i++){
                ...
                int product0=Integer.parseInt(product.get(0));
                int value=Integer.parseInt(map.get(product.get(i)).getValue());
                String type=map.get(product.get(i)).getKey();

                if(type.equals("0")){// same price
                    minPrice=Math.min(minPrice,value);
                } else if (type.equals("1")) {//% discount
                    minPrice=Math.min(minPrice,(product0 -((product0*value)/100 )));
                }else if(type.equals("2")){
                    minPrice=Math.min(minPrice,product0 - value);
                }
            }
            totalAmount+=minPrice;
        }
